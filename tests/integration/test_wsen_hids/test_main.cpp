...
/*
 * Integration validation for WSEN-HIDS on real STeaMi silicon.
 *
 * Verifies:
...
void wait_for_data_ready() {
    ...
    while (!sensor.dataReady()) {
        if (millis() - start > 3000) {
            TEST_FAIL_MESSAGE("Timeout waiting for WSEN-HIDS dataReady()");
        }
        delay(5);
    }
}

void test_wsen_hids_continuous_runtime_behavior() {
    ...
    TEST_ASSERT_FALSE_MESSAGE(all_frozen, "WSEN-HIDS readings remained frozen over all samples");
}
...
void setup() {
    ...
    UNITY_BEGIN();
    RUN_TEST(test_wsen_hids_continuous_runtime_behavior);
    UNITY_END();
}
